...
class PublicLaundryController extends AbstractController
{
    ...
    public function reviews(int $id, Request $request, LaundryRepository $laundryRepository, LaundryNoteRepository $laundryNoteRepository): JsonResponse
    {
        ...
        $companyName = trim((string) ($professional?->getCompanyName() ?? ''));
        if ($companyName !== '') {
            $professionalName = $companyName;
        } else {
            $proUser = $professional?->getUser();
            $firstName = trim((string) ($proUser?->getFirstName() ?? ''));
            $lastName = trim((string) ($proUser?->getLastName() ?? ''));
            $fallbackName = trim($firstName . ' ' . $lastName);
            if ($fallbackName !== '') {
                $professionalName = $fallbackName;
            }
        }
